...
//| User PC: Exness MT5 — master trades auto copy                    |
//|                                                                  |
//| ASAAN SETUP: PumpingBotSetup.bat chalao (installer sab khud kare) |
//| YA manual: Experts folder + token / config file                  |
//| Rozana: MT5 open + login + AutoTrading ON                        |
//+------------------------------------------------------------------+
...
input string InpToken     = "";          // optional — installer config file se bhi aa sakta hai
...
bool LoadConfigFile()
{
   // Installer writes: MQL5\Files\pumpingbot_config.txt
   // SERVER=https://...
   // TOKEN=hex...
   int h = FileOpen("pumpingbot_config.txt", FILE_READ|FILE_TXT|FILE_ANSI);
   if(h == INVALID_HANDLE)
      return false;
   while(!FileIsEnding(h))
   {
      ...
      if(StringFind(line, "SERVER=") == 0)
      {
         g_base = StringSubstr(line, 7);
         StringTrimLeft(g_base);
         StringTrimRight(g_base);
      }
      else if(StringFind(line, "TOKEN=") == 0)
      {
         g_token = StringSubstr(line, 6);
         StringTrimLeft(g_token);
         StringTrimRight(g_token);
      }
   }
   FileClose(h);
   return (StringLen(g_token) >= 8);
}
...
bool HttpGet(const string url, string &out)
{
   char data[];
   char result[];
   string headers = "User-Agent: PumpingBotEA/3.35\r\n";
   ResetLastError();
...
int OnInit()
{
   ...
   // Installer config overrides empty / default inputs
   LoadConfigFile();
   if(StringLen(InpToken) >= 8)
      g_token = InpToken;
   if(StringLen(InpServerUrl) > 8 && InpServerUrl != "")
   {
      // keep input server if user set it; config SERVER used when input is default-ish
   }
   // If config loaded SERVER and input still default, prefer config when g_base empty from file overwrite
   // Re-read: LoadConfigFile sets g_base if SERVER= present
   if(StringLen(g_base) < 8)
      g_base = InpServerUrl;
   ...
   {
      Alert("PumpingBot: Token nahi mila. PumpingBotSetup.bat chalao YA InpToken paste karo.");
      return INIT_FAILED;
   }
